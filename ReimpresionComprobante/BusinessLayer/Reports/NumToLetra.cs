...
namespace ReimpresionComprobante.BusinessLayer.Reports
{
    class NumToLetra
    {
        ...
        /// </summary>
        /// <param name="entero">Cantidad Entera</param>
        /// <param name="decimales">Cantidad Decimales</param>
        /// <param name="tipo">tipo de Moneda Cantidad</param>
        /// <returns></returns>
        public static string NumeroALetras(string entero, string decimales, string tipo)
        {
            string res = string.Empty;
            string dec = string.Empty;
            int numEntero = Convert.ToInt32(entero);
            //ReadWriteConfigFile readConfig = new ReadWriteConfigFile();
            if (tipo == "PESOS")
            {
                dec = " CON " + decimales + "/100 M.N.)";
            }
            else
            {
                if (tipo == "DOLARES")
                {
                    ...
                    //try
                    //{
                    //    _ClaveMA = readConfig.getCurrentAppConfigValue(ReadWriteConfigFile.configFile, "MonedaAmericanaEnPDFCFDI");
                    //}
                    //catch
                    //{
                    //    _ClaveMA = "M.A.";
                    //}
                    dec = " CON " + decimales + "/100 " + _ClaveMA + ")";
                }
            }

            res = "(" + NumeroALetras(Convert.ToDouble(numEntero)) + " " + tipo + dec;
            return res;
        }
        ...
        private static string NumeroALetras(double value)
        {
            ...
            if (value == 0)
                ...
                Num2Text = NumeroALetras(Math.Truncate(value / 10) * 10) + " Y " + NumeroALetras(value % 10);
            else if (value == 100)
                Num2Text = "CIEN";
            else if (value < 200)
            ...
            else if (value < 1000000000000)
            {
                ...
                Num2Text = "UN BILLON " + NumeroALetras(value - Math.Truncate(value / 1000000000000) * 1000000000000);
            else
            {
